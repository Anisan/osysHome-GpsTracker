...
from plugins.GpsTracker.utils import *
    ...
    def addGpsPosition(self, device:str, lat:float, lon:float, alt:float = None, accuracy:float = None, speed:float = None, battery:float = None, charging:bool = None, provider:str=None, address:str = None, added:datetime = None):
        with session_scope() as session:
            ...
            if device_rec.linked_object:
                updatePropertyThread(device_rec.linked_object + ".lat", lat)
                updatePropertyThread(device_rec.linked_object + ".lon", lon)
                updatePropertyThread(device_rec.linked_object + ".alt", alt)
                updatePropertyThread(device_rec.linked_object + ".location", current_location)
                updatePropertyThread(device_rec.linked_object + ".home", is_home)
                updatePropertyThread(device_rec.linked_object + ".address", address)
                updatePropertyThread(device_rec.linked_object + ".home_distance", distance)
                updatePropertyThread(device_rec.linked_object + ".battery", battery)
                updatePropertyThread(device_rec.linked_object + ".isCharging", charging)
